...
define HOST_SWIFT_BUILD_CMDS
	...
	# one. build-script's own C/C++/Swift builds all pass an explicit
	# compiler, but late in the same run swift-sdk-generator shells out to the
	# system swift-run to build the WebAssembly SDK, and that invokes SwiftPM,
	# which auto-enables indexing-while-building and hands "clang" (resolved
	# from PATH) an -index-store-path flag. A distro clang without
	# libIndexStore rejects it outright:
	#
	...
	@if [ ! -d "$(SWIFT_LLVM_DIR)" ]; then \
		(cd $(SWIFT_SOURCE_DIR) \
			&& export PATH="$(SWIFT_SOURCE_DIR)/build/buildbot_linux/llvm-linux-$(shell uname -m)/bin:$$PATH" \
