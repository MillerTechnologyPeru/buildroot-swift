### Swift ASN.1 (SwiftASN1)
SWIFT_ASN1_VERSION = 1.3.2
SWIFT_ASN1_SITE = $(call github,apple,swift-asn1,$(SWIFT_ASN1_VERSION))
SWIFT_ASN1_LICENSE = Apache-2.0
SWIFT_ASN1_LICENSE_FILES = LICENSE.txt
SWIFT_ASN1_INSTALL_STAGING = YES
SWIFT_ASN1_INSTALL_TARGET = YES
SWIFT_ASN1_SUPPORTS_IN_SOURCE_BUILD = NO
SWIFT_ASN1_DEPENDENCIES = swift swift-foundation

SWIFT_ASN1_BUILDDIR = $(SWIFT_ASN1_SRCDIR)/build

# SwiftASN1 links Foundation and dispatch on non-Darwin, so point find_package
# at the config packages emitted in the corelib build trees (same as xctest).
SWIFT_ASN1_CONF_OPTS += \
	-DCMAKE_Swift_FLAGS=${SWIFTC_FLAGS} \
	-DCMAKE_Swift_FLAGS_DEBUG="" \
	-DCMAKE_Swift_FLAGS_RELEASE="" \
	-DCMAKE_Swift_FLAGS_RELWITHDEBINFO="" \
	-Ddispatch_DIR="$(LIBSWIFTDISPATCH_BUILDDIR)/cmake/modules" \
	-DFoundation_DIR="$(SWIFT_FOUNDATION_BUILDDIR)/cmake/modules" \

define SWIFT_ASN1_CONFIGURE_CMDS
	(mkdir -p $(SWIFT_ASN1_BUILDDIR) && \
	cd $(SWIFT_ASN1_BUILDDIR) && \
	rm -f CMakeCache.txt && \
	PATH=$(BR_PATH):$(SWIFT_NATIVE_PATH) \
	$(SWIFT_ASN1_CONF_ENV) $(BR2_CMAKE) -S $(SWIFT_ASN1_SRCDIR) -B $(SWIFT_ASN1_BUILDDIR) -G Ninja \
		-DCMAKE_INSTALL_PREFIX="/usr" \
		-DBUILD_SHARED_LIBS=ON \
		-DCMAKE_BUILD_TYPE=$(if $(BR2_ENABLE_RUNTIME_DEBUG),Debug,Release) \
		-DCMAKE_C_COMPILER=$(SWIFT_NATIVE_PATH)/clang \
		-DCMAKE_C_FLAGS="-w -fuse-ld=lld -target $(SWIFT_TARGET_NAME) --sysroot=$(STAGING_DIR) $(SWIFT_EXTRA_FLAGS) -I$(STAGING_DIR)/usr/include -B$(STAGING_DIR)/usr/lib -B$(HOST_DIR)/lib/gcc/$(GNU_TARGET_NAME)/$(call qstrip,$(BR2_GCC_VERSION)) -L$(HOST_DIR)/lib/gcc/$(GNU_TARGET_NAME)/$(call qstrip,$(BR2_GCC_VERSION))" \
		-DCMAKE_C_LINK_FLAGS="-target $(SWIFT_TARGET_NAME) --sysroot=$(STAGING_DIR)" \
		$(SWIFT_ASN1_CONF_OPTS) \
	)
endef

define SWIFT_ASN1_BUILD_CMDS
	(cd $(SWIFT_ASN1_BUILDDIR) && ninja)
endef

# See swift-system.mk for why we hand-copy instead of `ninja install`.
define SWIFT_ASN1_INSTALL_STAGING_CMDS
	find $(SWIFT_ASN1_BUILDDIR) -name '*.so' -type f -exec cp -f {} $(STAGING_DIR)/usr/lib/swift/linux/ \;
	find $(SWIFT_ASN1_BUILDDIR) \( -name '*.swiftmodule' -o -name '*.swiftdoc' -o -name '*.swiftsourceinfo' -o -name '*.abi.json' \) -type f -exec cp -f {} $(STAGING_DIR)/usr/lib/swift/linux/$(SWIFT_TARGET_ARCH)/ \;
endef

define SWIFT_ASN1_INSTALL_TARGET_CMDS
	find $(SWIFT_ASN1_BUILDDIR) -name '*.so' -type f -exec cp -f {} $(TARGET_DIR)/usr/lib/ \;
endef

$(eval $(generic-package))
