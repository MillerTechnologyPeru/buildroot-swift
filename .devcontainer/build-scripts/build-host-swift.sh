...
do
    if [[ "$i" != "$SWIFT_TARGET_ARCH" ]]; then
        ...
        if [ ! -d "$BUILDROOT_OUTPUT_ARCH/build/host-swift-6.3.3" ]; then
            echo "Generating host tools symlink for $i"
            ln -s ../../$SWIFT_TARGET_ARCH/build/host-swift-6.3.3 ./host-swift-6.3.3
            rm $BUILDROOT_OUTPUT_ARCH/build/host-swift-6.3.3/.stamp_host_installed
        fi
    fi
done